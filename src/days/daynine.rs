...
pub(crate) fn daynine1(filepath: &str) -> Result<i32, String> {
    ...
    let last_diff_sq = initial_seq.map(|sequence| last_differences(sequence.collect_vec()));
    let mut sum: i32 = 0;
    for mut sequence in last_diff_sq {
        sequence.reverse();
...
pub(crate) fn daynine2(filepath: &str) -> Result<i32, String> {
    let lines = read_file(filepath).map_err(|_| "Error reading file")?;
    let initial_seq = lines.iter().map(|line| {
        line.split_whitespace()
            .filter_map(|num_str| num_str.parse::<i32>().ok())
    ...
    let last_diff_sq = initial_seq.map(|sequence| first_differences(sequence.collect_vec()));
    let mut sum: i32 = 0;
    for mut sequence in last_diff_sq {
        sequence.reverse();
        for i in 1..sequence.len() {
...
fn first_differences<T>(sequence: Vec<T>) -> Vec<T>
where
    T: std::ops::Sub<Output = T> + Copy + PartialEq + From<u8>,
{
    iter::successors(Some(sequence), |seq| {
